...
**Phase 3 — Generate report (terminal via format script)**: Pipe the curated JSON array through the format script at `scripts/format_report.py` (in the skill directory):
```bash
python3 <skill_dir>/scripts/format_report.py < /tmp/vndb_curated.json
```

**Phase 3 — Generate report (terminal via format script)**: Pipe the curated JSON array through the format script at `scripts/format_report.py` (in the skill directory). Alternatively, use the combined pipeline `scripts/curate_pipeline.py` which runs Phase 1-3 in one shot:
...
6. **Check page 2 of Phase 1 for Chinese originals with descriptions**: In a typical month with 40+ Phase 1 results, page 2 often contains additional Chinese-original games, some of which DO have VNDB descriptions (unlike the description-less majority on page 1). In July 2026, page 2 of Phase 1 yielded 14 more Chinese originals, 3 of which had descriptions (东岚男子体育学院 越线, 局外人局内人, One Day). Page 2 candidates with descriptions should be preferred over page 1 entries without descriptions.
