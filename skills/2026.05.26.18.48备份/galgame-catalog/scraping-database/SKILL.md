...
- **⚠️ 术语陷阱**："新番"是动画（anime）术语，Galgame/视觉小说的播报标题必须用「今日Galgame～」而非「新番速报」。VNDB 返回的是视觉小说（visual novel），与动画新番是完全不同的媒介。

## 六、Cron 播报格式规范\n\nGalgame/轻小说每日速报的输出格式有严格要求，详见 [`references/cron-news-format.md`](references/cron-news-format.md)。\n\n关键点：标题用「今日Galgame～」而非「新番速报」；书名用简称外号；编号列表不加简介。\n\n## 七、轻小说情报采集（轻之国度 lightnovel.cn）（轻之国度 lightnovel.cn）
