...
### 🔧 Step 0: Configure Auxiliary Vision (ONE-TIME FIX)

**Before falling back to workarounds, configure Hermes' auxiliary vision provider.** This routes `vision_analyze` calls to a vision-capable model, and the fix persists across sessions.

```bash
# Use a vision-capable model through the API relay (or any OpenAI-compatible endpoint)
...
Then add the custom provider to `~/.hermes/config.yaml`:
```yaml
providers:
  apirelay:
...
After restart, `vision_analyze` will use the vision model transparently. The main chat model (e.g., DeepSeek) stays unchanged. See `references/auxiliary-vision-setup.md` for the full guide.

### Fallback Chain (try in order, if Step 0 not done)

1. **`vision_analyze`** — Always try first. If auxiliary vision is configured, this just works.
...
**Restart:** In testing (2026.5.26), `hermes config set auxiliary.vision.*` took effect immediately without a gateway restart — the next `vision_analyze` call in the same session used the new config. If it doesn't work right away, restart the gateway.
...
### Image Generation & Sending (QQ)

For generating and sending images via QQ (Pillow → PNG → MEDIA: path), see [`references/qq-image-generation.md`](references/qq-image-generation.md). This is useful for sending custom emoji,表情包, or generated graphics to QQ users.
...
- When creating cron jobs that deliver messages, set `--deliver` to an explicit platform (e.g. `qqbot`). Do NOT use `origin` — it resolves to the session where the cron was created, which may be a different platform from where the user is currently active.

## Autonomous / Proactive Messaging

For cron-driven proactive check-ins (早安/午间/晚间 greetings, random conversation starters), see [`references/proactive-cron-messaging.md`](references/proactive-cron-messaging.md). This pattern uses Hermes cron jobs with personality-rich prompts to generate autonomous messages delivered via QQ/Discord/Telegram.

## Personality / Character Drift

When the agent drops its character voice during technical tasks (switching to cold documentary tone), the fix is a SOUL.md rewrite with hard enforcement rules. See [`references/soul-personality-enforcement.md`](references/soul-personality-enforcement.md) for the template and a real-world case study.
