...
| enso-os v0.7.0 | Hook 插件 | 错误学习 + 主动遗忘 + PAC 反思挑战。bash install.sh --target hermes |

## 已验证不可用的插件（硬件/环境限制）

| 插件 | ⭐ | 失败原因 | 替代方案 |
|------|------|---------|---------|
| hermes-local-vision (Blip) | 1 | 需要 GPU 跑 Qwen2.5-VL-7B（≥8GB VRAM），或 CPU ≥8GB RAM。轻量 ECS（1.6GB）无法运行 | API 中转站 + 百炼 qwen-vl-plus（参见 model-fallbacks skill） |
