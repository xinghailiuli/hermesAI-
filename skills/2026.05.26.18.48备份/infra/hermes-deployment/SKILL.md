...
### 主动搭话（Proactive Messaging）

部署完成后可创建「主动搭话」cron 任务，让 AI 在固定时间主动向用户发送问候，而非被动等用户提问。配合 enso-os 的 PAC 反思机制形成「日常关怀 + 深度反思」双模式。
详见 `references/cron-management.md` 的「主动搭话 Cron」章节。
