...
**Phase 1 — Chinese-first query** (run first to find games available in Chinese):
```json
{
  "filters": ["and",
    ["or",
...
### Steam fallback for missing descriptions (cron mode)

When VNDB returns `null` descriptions for Chinese-original or Chinese-supported games, **try extracting descriptions from Steam store pages** before falling back to "（暂无简介）".

Technique: search Steam by Chinese title, extract the `game_description_snippet` div:

```bash
# 1. Find Steam app ID by searching the Chinese title
app_id=$(curl -s --connect-timeout 10 --max-time 15 \
  "https://store.steampowered.com/search/?term=URL_ENCODED_CHINESE_TITLE" \
  2>/dev/null | grep -oP 'app/\d+' | head -1)

# 2. Fetch description from the app page
...
- Steam pages are rate-limited. Limit to 3-5 Steam lookups per cron run. Prefer games with VNDB descriptions first; only supplement the top 2-3 Chinese-original entries that lack VNDB descriptions.
...
- Steam search may fail for obscure indie games — skip gracefully if no app ID found.
