...
def main():
    ...
    while len(entries) >= MAX_BACKUPS:
        ...
        # Prefer subprocess.run for rm -rf: avoids potential shutil.rmtree
        # permission issues and is identical to shell behavior.
        subprocess.run(["rm", "-rf", oldest_path], check=True)
    ...
    print(f"Backup count: {matched} (total dirs: {len(remaining)})")
