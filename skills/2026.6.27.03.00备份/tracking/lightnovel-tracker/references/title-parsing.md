...
| `取代江户花魁后，我决定登上花街之巅 三` | 未知 (items) | — (三=3, 中文数字) |
| `义妹生活16` | 三河ごーすと (known) | 第16卷 |

### Volume number edge cases (from observed data)

- **Trailing bare number** `义妹生活16` → should match as "第16卷" via number-at-end pattern `(\\d+)$`. The number must be < 50 (to avoid false matches on page counts or years).
- **Chinese numeral marker** `取代江户花魁后，我决定登上花街之巅 三` → the "三" (3) is a Chinese numeral, not a digit. The current patterns only match Arabic digits, so this won't be detected as a volume. Consider adding Chinese numeral detection for `一二三四五六七八九十` if this proves common.
- **No space before number** `义妹生活16` has no space between title and volume number. The `(\\d+)$` pattern handles this correctly.
