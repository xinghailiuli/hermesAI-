...
```bash
# 用 token 通过 API 查用户仓库
curl -s -H "Authorization: token <token>" \
  https://api.github.com/user/repos?per_page=30 | \
  python3 -c "import sys,json; repos=json.load(sys.stdin); [print(f'{r[\"full_name\"]}') for r in repos]"

# 克隆
git clone https://github.com/<user>/<repo>.git /tmp/hermes-restore/
```
