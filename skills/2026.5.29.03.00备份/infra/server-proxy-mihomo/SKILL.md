...
`release-assets.githubusercontent.com` is a separate domain from `github.com` and `api.github.com`. Some airport nodes handle it differently — downloads consistently fail with `curl: (35) SSL_ERROR_SYSCALL` while API calls and git clone succeed on the same proxy.

**Diagnosis**: If `curl -x http://127.0.0.1:7897 https://github.com` returns 200 but `curl -L https://github.com/USER/REPO/releases/download/v1.0/file.tar.gz` fails with SSL_ERROR_SYSCALL, the proxy node is throttling/blocking the release CDN domain specifically.

**Workaround**: 
1. `git clone` the repo (small) + build from source if feasible
2. User downloads the binary on local PC → `scp` to server
3. NEVER use QQ file transfer for binaries — QQ wraps files in a non-extractable format (PK header but not valid zip/tar)
...
## Systemd User Service (Headless Server)
