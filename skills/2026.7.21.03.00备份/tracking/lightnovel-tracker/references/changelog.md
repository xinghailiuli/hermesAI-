...
## 2026-07-20 — "新站调试中" maintenance variant; detection updated

- **Site showing "新站调试中"** (new site debugging) instead of the previous "网站维护中" message. This is a new variant of the maintenance page — same ~25KB static HTML with no NUXT state, rendered identically on both `.fun` and `.cn` domains.
- **Updated maintenance detection** in `scripts/parse_and_report.py`: changed from single-string check to a list-based approach (`maintenance_signals = ['网站维护中', '新站调试中', '正在维护']`) so future variants are easy to add.
- **Updated SKILL.md** pitfall section to document the new "新站调试中" signal.

## 2026-07-13 — Fixed stale series_name param-resolution bug; maintenance detection confirmed

- **Fixed bug in `scripts/parse_and_report.py`**: The `series_name` param-reference condition was still using the old hardcoded `len(sn_raw) <= 3 and (sn_raw.isalpha() or sn_raw in ('aa','ab','ac','ad','ae','af','ag'))` pattern (broken for params beyond `ag`). The changelog from 2026-07-09 claimed this was already fixed, but the fix was never actually applied to the script file. Applied `len(sn_raw) <= 4 and sn_raw.isalnum()` — generic and future-proof.
- **Maintenance page detection confirmed working**: Site returned HTTP 503 with "网站维护中" across all access paths (`.fun` direct, `.cn` direct, `.cn` via proxy). The script's early-exit check (`if '网站维护中' in html[:500] and 'window.__NUXT__=' not in html`) correctly caught it with no false positives.
- **Noted redirect chain**: `lightnovel.fun/` now redirects to `www.lightnovel.fun/` before returning 503. The skill's Step 1 curl command already uses `-L` to follow redirects, so this doesn't break anything, but good to be aware of.

## 2026-07-09 — New author mappings: 天空的彼端 & 替子

- **Added author mappings**: `天空的彼端 → 弥生志郎` (空の彼方で, Fujimi Fantasia Bunko) and `替子 → 碧井ハル` (替子 -紅の誓約-) to both `scripts/parse_and_report.py` KNOWN_AUTHORS and `references/known-series-authors.md`.
- **Discovered latent bug in series_name resolution**: The `series_name` param-reference condition `len(sn_raw) <= 3 and (sn_raw.isalpha() or sn_raw in ('aa','ab','ac','ad','ae','af','ag'))` is hardcoded up to `ag` only. When NUXT params grow beyond 48 (into `ah`+), series_name resolution silently breaks, causing CN-web-novel detection from series_name to stop working. The fix is `len(sn_raw) <= 4 and sn_raw.isalnum()` — generic, future-proof. Applied to script.
- **Report**: Only 1 entry in the 3-day window (天空的彼端 02, now with author), 6 in near-miss (7~14d), 5 in week-older (14~21d). 2 CN entries filtered.

## 2026-07-08 — Bracket-author confusion (translator vs author) documented

- **New pitfall**: Rank entries' `[bracket]` content may be the **translator's name**, not the original author. Example: `[夜凪] 森林边上的小小魔女` had "夜凪" (translator) instead of 小石川うみ (author). Previously the skill incorrectly stated brackets always contained the author.
- Updated SKILL.md pitfall section and author extraction step to warn about this.
- Updated `references/author-research.md` with a dedicated subsection documenting the translator-bracket caveat and how to handle it.
