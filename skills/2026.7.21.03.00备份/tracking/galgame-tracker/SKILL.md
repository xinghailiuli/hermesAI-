...
- **Phase 1 often has more Chinese originals on page 2**: With a 3-month window, Phase 1 routinely returns 40+ results. Page 2 may contain additional Chinese original games — and crucially, some of them have VNDB descriptions (unlike the description-less majority on page 1). Always check at least page 2 of Phase 1 for Chinese originals with descriptions before relying entirely on page 1 entries without descriptions.
- Phase 2 (rated ≥ 60) pagination is less useful — it mostly returns English-only games with < 5 Chinese-supported entries per page.
...
### Approach A — execute_code + terminal (PREFERRED — three-phase pipeline)

This is the recommended three-phase pipeline that avoids the syntax-error issue of generating markdown inside execute_code while keeping query logic clean:

**Phase 1 — Query & analyze (execute_code)**: Use `execute_code` to query VNDB pages, inspect results, and identify candidates. Print intermediate analysis to stdout for review. Save the candidate fetch results to /tmp via `json.dump`:
```python
import json, urllib.request

def vndb_request(filters, fields=..., results=40, page=1):
    body = json.dumps({
...
# Phase 1: query page 1 + page 2
phase1_p1 = vndb_request(phase1_filters, page=1)
phase1_p2 = vndb_request(phase1_filters, page=2)
p1 = phase1_p1['results'] + phase1_p2['results']

# Phase 2: broader query
phase2 = vndb_request(phase2_filters)['results']

# Inspect, classify, select candidates...
candidate_ids = ['vXXXXX', ...]

# Batch-fetch details
details = {vid: fetch_vn(vid) for vid in candidate_ids}

# Save to /tmp for the next phase
with open('/tmp/vndb_detail.json', 'w') as f:
    json.dump(details, f, ensure_ascii=False, indent=2)
```

**Phase 2 — Curate & select (execute_code)**: Read the saved detail JSON, inspect descriptions, sexual scores, and rating info. Narrow to the final 8 entries, save as an ordered array:
```python
import json
with open('/tmp/vndb_detail.json', 'r') as f:
    details = json.load(f)

# Hand-curate the best entries (or use format_report.py's sort_key logic)
curated_ids = ['vXXXXX', 'vYYYYY', ...]  # ordered list
curated = [details[vid] for vid in curated_ids]

with open('/tmp/vndb_curated.json', 'w') as f:
    json.dump(curated, f, ensure_ascii=False, indent=2)
```

**Phase 3 — Generate report**: Pipe the curated JSON array through the format script:

```bash
python3 <skill_dir>/scripts/format_report.py < /tmp/vndb_curated.json
```

**One-shot pipeline (recommended for cron jobs)**: Instead of the multi-step execute_code approach, use the standalone pipeline script for a single terminal command:
```bash
# Step 1: Run the pipeline (queries VNDB, curates, saves to /tmp/vndb_curated.json)
python3 <skill_dir>/scripts/vndb_fetch_pipeline.py 2>/dev/null

# Step 2: Format the report
python3 <skill_dir>/scripts/format_report.py < /tmp/vndb_curated.json
```
The `vndb_fetch_pipeline.py` script handles both Phase 1 (Chinese-first, 3-month window, pages 1+2) and Phase 2 (English+Chinese rated, 2-month window), classifies results, and selects up to 8 candidates prioritizing Chinese originals with descriptions first. It saves intermediate data to `/tmp/vndb_raw_phase1.json` and `/tmp/vndb_raw_phase2.json` for optional inspection. This is cleaner than the multi-execute_code approach because it avoids the sandbox/terminal filesystem isolation issue entirely.

Alternatively, use the combined pipeline `scripts/curate_pipeline.py` which runs Phase 1-3 in one shot:

```bash
# One-shot pipeline (queries + curates + saves to /tmp/vndb_curated.json)
python3 <skill_dir>/scripts/curate_pipeline.py
# Then format
python3 <skill_dir>/scripts/format_report.py < /tmp/vndb_curated.json
```

**Advantages of this three-phase approach**: (a) No filesystem I/O conflicts between sandbox and terminal. (b) JSON as the bridge format avoids string-escaping issues with VNDB descriptions (curly quotes, multi-line). (c) The format script handles BBCode cleaning, Chinese title extraction, badges, and summary automatically. (d) Easy to inspect intermediate data between phases. (e) The `curate_pipeline.py` script combines Phases 1-3 into a single terminal command for cron jobs.
...
- **Network constraints**: Steam may be unreachable from certain cloud/VPS environments (`URLError: timed out`, `SSL: UNEXPECTED_EOF_WHILE_READING`). If Steam consistently fails (timeout or SSL errors), skip fallback entirely — the server cannot reach Steam's CDN. Set short timeouts (10-15s).

### Curation rules

1. **Prioritize Chinese-supported games** — Phase 1 results (games listing zh-Hans/zh-Hant in their languages) are preferred over Phase 2 multi-lang results
...
5. **Target mix for cron reports**: Aim for 3-4 Chinese originals (even without descriptions) + 2-3 Chinese-supported games with notable ratings/platforms + 1-2 multi-platform or AAA releases. This prevents the report from being either too sparse or too padded. Chinese originals belong at the top even without descriptions — their presence is the primary value for a Chinese audience. **Phase 1 can yield 40 results** even with a 3-month window; most will be multi-language games that happen to include Chinese, not true Chinese originals. Filter aggressively for `languages = ['zh-Hans']` to find the real originals.
6. **Check page 2 of Phase 1 for additional Chinese originals**: With a 3-month window, Phase 1 routinely returns 40+ results on page 1, with page 2 adding more. However, Chinese originals rarely have VNDB descriptions regardless of which page they're on — in a typical run, only ~25% (e.g. 4 out of 16) of Chinese original entries have descriptions across both pages combined. Fetch page 2 for quantity (more candidates to choose from), not for description quality. Page 2 may have Chinese originals that are newer or more recently indexed.
7. **Phase 2 is a supplement, not an independent pool**
8. **Extract Chinese title**: check zh-Hans first, then zh-Hant, then zh from the titles array. Do NOT rely on the top-level title or alttitle fields — some Chinese VNs have their Chinese title stored as pinyin in title (e.g. "Ranshang Ni de Yanse" = "染上你的颜色") and alttitle may be empty or romanized too.
9. **Cover image URL**: VNDB images are at https://t.vndb.org/cv/{xx}/{xxxxx}.jpg
10. **Watch for pinyin-masked titles**: If a game's title looks like romanized pinyin, check titles entries with lang of zh-Hans/zh-Hant/zh to find the proper Chinese title. Use the Chinese title as the display name, not the pinyin title field.
11. **Missing descriptions — cron mode**: When running as a cron job (no user present), do NOT fabricate Chinese descriptions. Display the English description (cleaned of BBCode/markup) for games that lack Chinese text. For games with neither description nor Chinese data, display "（暂无简介）". Never make up content in autonomous mode — the report is auto-delivered and cannot be corrected.
...
## Step 4 — Reference sites (supplementary)
...
- `scripts/vndb_fetch_pipeline.py` — One-shot pipeline for cron jobs: queries VNDB Phase 1 + Phase 2, classifies, curates 8 entries, saves to `/tmp/vndb_curated.json`
...
- **Final report generation**: Avoid generating markdown output inside `execute_code` — VNDB descriptions contain special characters (curly quotes, multi-line strings) that break Python string formatting. Use the three-phase pipeline instead: (Phase 1) query + fetch details in `execute_code`, save to `/tmp/vndb_detail.json`; (Phase 2) curate selection in `execute_code`, save curated array to `/tmp/vndb_curated.json`; (Phase 3) run the format script via `terminal`: `python3 <skill_dir>/scripts/format_report.py < /tmp/vndb_curated.json`. This JSON-bridge approach avoids all string-escaping issues.
- `image` can be `None`: Some VNs in VNDB have `"image": null` (no cover art). The `format_report.py` script must handle this case: use `vn.get('image') or {}` as a fallback default dict, and check `isinstance(img_obj, dict)` before calling `.get()` on it. Without this handling, the script crashes with `AttributeError: 'NoneType' object has no attribute 'get'`. Both `url` and `sexual` access patterns need this guard.
- **Steam fallback timeout from cloud servers**: Steam's CDN may block or fail from cloud/VPS IP ranges. Common errors: `URLError: timed out` at 15s, `SSL: UNEXPECTED_EOF_WHILE_READING`. Document missing descriptions as "（暂无简介）" after one quick attempt; do not retry Steam fallback in cron mode if the first attempt times out.
