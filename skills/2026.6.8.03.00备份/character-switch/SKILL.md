...
角色文件从 GitHub 仓库 `xinghailiuli/hermesAI-/阿罗娜角色/` 恢复（按时间戳子文件夹存放）。
完整备份/恢复流程见 `hermes-deployment` 技能（`references/github-backup-restore.md`）。

## 注意事项

- `arona-soul.md`（阿罗娜）是默认 Hermes Agent 系统提示的定制版，仅有 513 字节，实际人设主要靠 `display.personality: kawaii` 驱动
- `plana-soul.md`（普拉娜）包含完整中文人设规则：称呼「老师」、语气轻柔带软糯语气词、安静陪伴型
- 两个角色的 `config.yaml` 主要差异在 `display.personality`（kawaii vs plana）和 `display.language`