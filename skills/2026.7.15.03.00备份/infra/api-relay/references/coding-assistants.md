...
## Reasonix (DeepSeek-native coding agent, 22k+⭐)

**What it is**: `esengine/DeepSeek-Reasonix` — a Go-based, zero-dependency terminal
coding agent engineered specifically for DeepSeek's prefix-cache mechanism. Single
static binary (~25MB), installs via `npm i -g reasonix`. v1.8.0+.

**Key differentiator**: Unlike Claude Code or Codex which are retrofitted for
DeepSeek, Reasonix is built around cache stability from the ground up. It freezes
the prompt prefix so DeepSeek's automatic cache always hits — measured 64% cheaper
on real-world traffic. The `soft_compact_ratio` / `compact_ratio` / `compact_force_ratio`
settings control when context trimming kicks in without breaking the cache prefix.

### Install

```bash
sudo npm install -g reasonix    # auto-pulls native Go binary
reasonix --version               # v1.8.0
```

### Configure for API relay

Config path: `~/.reasonix/config.toml` (not `~/.config/reasonix/` — v1.8+ resolved path).
...
temperature       = 0.0
auto_plan         = "off"
soft_compact_ratio  = 0.5   # start trimming at 50% context
compact_ratio       = 0.8   # aggressive compact at 80%
compact_force_ratio = 0.9

[[providers]]
name           = "deepseek"
kind           = "openai"
base_url       = "http://127.0.0.1:8847/v1"       # Flask relay, NOT 8848 (WS bridge)
models         = ["deepseek-reasoner"]              # must match relay's model ID
default        = "deepseek-reasoner"
api_key_env    = "RELAY_TOKEN"                      # local_token, NOT upstream DeepSeek key
...
**Auth token**: Reasonix sends `Authorization: Bearer <RELAY_TOKEN>` to the
relay. The relay's `check_auth()` only matches against `config.json` →
`auth.local_token` (e.g., `sk-local-apirelay-2026`). Using the actual
DeepSeek API key (`DEEPSEEK_API_KEY`) causes **401 Unauthorized**.

Fix: Export the relay's local token to an env var:
```bash
export RELAY_TOKEN=sk-local-apirelay-2026    # add to ~/.bashrc
```

**Model name**: Must match exactly what the relay's `/v1/models` returns.
For V4-Pro it's `deepseek-reasoner`, NOT `deepseek-v4-pro`.

**Streaming required**: Reasonix always uses SSE streaming. The relay's
`/v1/chat/completions` endpoint must support `stream: true`. Before mid-2026
the endpoint hardcoded `stream: false` — it has since been updated to proxy
OpenAI SSE natively.

**Port**: `base_url` must point to the Flask relay (8847), not the Codex
WebSocket bridge (8848). The WS bridge only handles WebSocket upgrade to
`/v1/responses`.
...
```bash
# One-shot task
reasonix run "fix the test failures" --yes

# Interactive chat
reasonix chat

# With specific model
reasonix run --model deepseek-reasoner "explain this code"
```

Cache hit rate on first turn ~86% (768/886 tokens cached) with V4-Pro — drops
significantly if the prefix changes between turns (keep `temperature: 0.0` and
avoid rotating system prompts).

### Comparison

| | Reasonix | Codex | Claude Code |
|---|---|---|---|
| Binary size | ~25MB (Go) | ~300MB (npm deps) | ~200MB (npm deps) |
| API format | OpenAI native | Responses API (WS) | Anthropic |
| Cache strategy | Built-in prefix freezing | None | None |
| DeepSeek optimized | ✅ Yes (primary target) | ❌ (retrofit) | ❌ (retrofit) |
| Streaming | Always | WS events | SSE |
| Sandbox | workspace | bwrap (configurable) | read-only |
