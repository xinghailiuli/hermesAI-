...
### Git LFS push 报 TLS / locksverify 错误

LFS migrate 后 push 可能遇到两种错误：

**错误 1**: `Remote "origin" does not support the Git LFS locking API`
```bash
cd ~/hermesAI-backup
git config --local lfs.locksverify false
```

**错误 2**: `gnutls_handshake() failed: The TLS connection was non-properly terminated`
— 代理（mihomo）可能干扰 Git LFS 的 HTTPS 连接。修复：
```bash
