...
安装新插件后必须备份到 GitHub。详见 `references/plugin-management.md` 和 `references/github-backup-sync.md`。

### GitHub Token 过期

备份同步到 GitHub 时 token 过期导致 push 失败，需要重新生成 PAT。详见 `references/github-backup-sync.md`。
