...
用 `cronjob(action="create", ...)` 工具从 `jobs.json` 重建每个任务，不能用 `hermes cron create` CLI（不支持 `no_agent`/`script` 等参数）。
...
标准任务清单见 `cron-management.md` 的「标准 Cron 任务清单」节。GitHub 每日备份任务需额外指定 `no_agent=true` 和 `script="github-daily-backup.py"`。
